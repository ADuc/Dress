...
public class EditorObjExporter : ScriptableObject
{
		...
		using (StreamWriter sw = new StreamWriter(folder + "/" + filename + ".mtl")) 
		{
			foreach( KeyValuePair<string, ObjMaterial> kvp in materialList )
			{
				...
				if (kvp.Value.textureName != null )
				{
					...
					if(relativeFile.Trim().Equals(""))
						relativeFile = "Image.jpg";
	...
	public static void ExportEachToSingle(GameObject obj){
		...
		Texture2D texture = (Texture2D)obj.GetComponent<MeshRenderer> ().material.mainTexture;
		SaveTextureToFile (texture, "Image");
		MeshToFile(mf, targetFolder, name);
	}
	...
	public static void SaveTextureToFile( Texture2D texture, string fileName)
	{
		...
		File.WriteAllBytes(targetFolder+ "/" + fileName +".jpg", bytes);
	}
}