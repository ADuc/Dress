...
public class ObjReader : MonoBehaviour {
	...
	private string overMtlName, overPngName;
	...
	public ObjData ConvertFileAsync (string objFilePath, bool useMtl, Material standardMaterial, string mtlName, string pngName) {
		this.overMtlName = mtlName;
		this.overPngName = pngName;
		return ConvertFileAsync (objFilePath, useMtl, standardMaterial, null);
	}
	...
	private string GetFileName (string line, string token) {
		...
		if (overMtlName != null && token.Equals ("mtllib")) {
			return overMtlName;
		}
		if (overPngName != null && token.Equals ("map_Kd")) {
			return overPngName;
		}
		return fileName;
	}
